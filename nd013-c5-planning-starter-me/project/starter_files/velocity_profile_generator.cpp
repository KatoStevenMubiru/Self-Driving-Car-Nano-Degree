...
4                                               const double& v_f,
5                                               const double& a) const {
6  double d{0.0};
7  // TODO-calc distance: use one of the common rectilinear accelerated equations of motion to calculate the distance traveled while going from v_i (initial velocity) to v_f (final velocity) at a constant acceleration/deceleration "a".
8  d = (v_f * v_f - v_i * v_i) / (2 * a);  // Assuming v_f is the final velocity (0 for full stop)
9  return d;
10}
...
13                                                  const double& a,
14                                                  const double& d) const {
15  double v_f{0.0};
16  // TODO-calc final speed: Calculate the final distance.
17  double disc = v_i * v_i + 2 * a * d;  // Discriminant
18  if (disc < 0.0) {
19    v_f = 0.0;  // If the discriminant is negative, the final speed should be zero.
20  } else {
21    v_f = std::sqrt(disc);  // Final speed calculation
22  }
23  return v_f;
24}
25
